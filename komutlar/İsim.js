...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get("767786534748291104")&&!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));
    ...
    if(rAd.user.username.includes("♩")){
      await rAd.setNickname("マ " + reason);(e => console.log(e.message))
    }

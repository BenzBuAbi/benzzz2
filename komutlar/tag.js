...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get("767786514041012284") &&!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));
    


  var user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));

  var Crew = message.guild.roles.get("767786551639277579")
if(!user) return message.reply ("Lütfen bir kullanıcı etiketleyiniz").then(m => m.delete(5000));
   let takmaad =  client.guilds.get("762350962097520678").members.get(user.user.id).displayName
  ...
  if(!user.roles.has(Crew.id)){
      ...
      if(user.user.username.includes("マ")){
        let değişeceksembol2 = takmaad.replace(/★/g, "マ");
...
**`)
  ...
  message.channel.send("Etiketlenen Kullanıcının İsminde **マ** Yok!").then(m => m.delete(10000));
}
...
**`)
  ...
  message.channel.send("Etiketlenen Kullanıcının İsminde Zaten **マ** Var!").then(m => m.delete(10000));
}
